...
command! -nargs=+ DBExecSQLExplain  :call dbext#DB_execSql("EXPLAIN " . <q-args>)
...
if !exists(':DBExecSQLExplain')
    command! -nargs=0 DBExecSQLExplain
                \ :call dbext#DB_execSql(dbext#DB_parseQuery("EXPLAIN " . dbext#DB_getQueryUnderCursor()))
    nmap <unique> <script> <Plug>DBExecSQLExplain :DBExecSQLExplain<CR>
endif
if !exists(':DBExecSQL')
...
if !hasmapto('<Plug>DBExecSQLExplain') && !hasmapto('<Leader>see', 'n')
    nmap <unique> <Leader>see <Plug>DBExecSQLExplain
endif
